(function () {
  ...
  const API_KEY_STORAGE = 'cike_api_key';
  const API_PROVIDER_STORAGE = 'cike_api_provider';
  const PROVIDERS = {
    ...
    claude: { name: 'Claude', placeholder: 'sk-ant-...', url: 'https://console.anthropic.com/settings/keys' },
  };
  ...
  function getApiKey() {
    return localStorage.getItem(API_KEY_STORAGE + '_' + getProvider()) || '';
  }

  function saveApiKey(key) {
    const k = API_KEY_STORAGE + '_' + getProvider();
    if (key) {
  ...
  let currentBreathingText = getBreathingText();
  ...
  function updateProgressRing() {
    ...
    const displayTotal = Math.max(total, 3);
    const progress = total > 0 ? (allDone ? 1 : doneCount / displayTotal) : 0;
    const offset = RING_CIRCUMFERENCE * (1 - progress);

    ringFill.style.strokeDashoffset = offset;
    progressText.textContent = `${doneCount}/${displayTotal}`;
  ...
  function render() {
    ...
    breathingEl.textContent = currentBreathingText;
  ...
  function createTaskEl(task, isToday) {
    ...
    if (isToday && !task.done) {
      ...
      moreBtn.onclick = e => {
        e.stopPropagation();
        const wasOpen = li.classList.contains('show-more');
        document.querySelectorAll('li.show-more').forEach(el => el.classList.remove('show-more'));
        if (!wasOpen) li.classList.add('show-more');
      };
  ...
  function completeTask(id) {
    ...
    if (task.done) {
      task.started = true;
      if (!wasDone && !task._completedOnce) {
        task._completedOnce = true;
        logCompletion();
      }
    }
    save(data);
  ...
  function promoteToToday(id) {
    const activeTodayCount = data.tasks.filter(t => t.isToday && !t.done && !t._pendingDelete).length;
  ...
  function deleteTask(id) {
    const task = data.tasks.find(t => t.id === id);
    if (!task || task._pendingDelete) return;
  ...
  function updateApiKeyStatus() {
    ...
    keyLink.href = PROVIDERS[provider].url;
    keyLink.textContent = '获取 ' + PROVIDERS[provider].name + ' API Key';
    if (key) {
      apiKeyStatus.textContent = '✓ ' + PROVIDERS[provider].name + ' 已配置';
      apiKeyStatus.className = 'api-key-status configured';
      apiKeyClear.classList.remove('hidden');
    } else {
      apiKeyStatus.textContent = PROVIDERS[provider].name + ' 未配置';
      apiKeyStatus.className = 'api-key-status';
      apiKeyClear.classList.add('hidden');
    }
  }
  ...
  // --- AI Suggest Step ---
  const AI_SYSTEM_PROMPT = `你是一个帮助用户克服拖延的任务拆解助手。
用户会给你一个任务，你需要建议一个具体的第一步。

规则：
- 这一步必须 5 分钟内能做完
- 越具体越好，具体到不需要思考就能动手
- 只回复这一步的内容，不要解释、不要编号、不要引号
- 用中文回复
- 控制在 20 字以内`;

  // --- Local template fallback (no API Key needed) ---
  const STEP_TEMPLATES = [
    { keywords: ['写', '论文', '文章', '报告', '文档', '作文', '方案', '邮件'], step: '打开文档，写下标题和第一句话' },
    { keywords: ['读', '看书', '阅读', '学习', '复习'], step: '翻开第一页，读完前两段' },
    { keywords: ['整理', '收拾', '打扫', '清理'], step: '先处理眼前最碍事的一样东西' },
    { keywords: ['运动', '锻炼', '健身', '跑步', '瑜伽'], step: '换上运动服，做 5 个拉伸动作' },
    { keywords: ['打电话', '联系', '沟通', '回复', '消息'], step: '打开对话框，先打一句"你好"' },
    { keywords: ['买', '购物', '采购'], step: '打开购物 app，搜索你要买的第一件' },
    { keywords: ['做饭', '烧菜', '下厨'], step: '把第一样食材拿出来放在台面上' },
    { keywords: ['代码', '编程', '开发', 'bug', '修复'], step: '打开编辑器，找到要改的那个文件' },
    { keywords: ['设计', '画', 'PPT', '幻灯片'], step: '新建文件，写下第一页的标题' },
    { keywords: ['面试', '准备', '考试'], step: '写下你最有把握的三个点' },
    { keywords: ['起床', '早起'], step: '先坐起来，双脚放到地上' },
    { keywords: ['申请', '注册', '报名'], step: '打开网页，找到申请入口' },
  ];
  ...
  function localSuggestStep(taskText) {
  ...
  async function aiSuggestStep(taskText) {
    const apiKey = getApiKey();
    const provider = getProvider();
    if (!apiKey) {
      showToast('请先配置 ' + PROVIDERS[provider].name + ' API Key');
      showSettings();
      return null;
    }

    if (provider === 'gemini') {
      return aiCallGemini(apiKey, taskText);
    }
    if (provider === 'openai') {
      return aiCallOpenAI(apiKey, taskText);
    }
    return aiCallClaude(apiKey, taskText);
  }

  async function aiCallGemini(apiKey, taskText) {
  ...
  async function aiCallOpenAI(apiKey, taskText) {
  ...
  async function aiCallClaude(apiKey, taskText) {
  ...
  aiSuggestBtn.addEventListener('click', async () => {
    ...
    aiSuggestBtn.textContent = '正在思考…';
    aiSuggestBtn.classList.add('loading');

    try {
      const suggestion = await aiSuggestStep(task.text);
      if (suggestion) {
        firststepInput.value = suggestion;
      ...
      aiSuggestBtn.textContent = '✨ 帮我想一步';
  ...
  setInterval(() => {
    currentBreathingText = getBreathingText();
    breathingEl.textContent = currentBreathingText;
  }, 30000);
  ...
  document.addEventListener('click', e => {
    // Close all open show-more menus when clicking outside
